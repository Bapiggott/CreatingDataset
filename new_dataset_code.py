...
# Due to the size of the json, moving it in chunks is more sufficient
...
# Method to see which protocol is being used, moves it to the respected directory
...
def process_files(target_directory, json_files):
    ...
    # uses max_workers=5, might need to lower or raise depeneding on computer
...
def run_move_jsons(target_directory):
    ...
    #ensures no json files are empty
...
# method that is giving us the trouble
# we narrowed it down to just tcp but if error is fixed, will work for all 
def filtered_jsons(input_directory, output_directory, chunk_size=100):
    # Create the output directory if it doesn't exist
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    filename = 'capture_66-0001.pcap.json'
...
def pcaps2jsons(output_directory):
    ...
    with tqdm(total=len(pcap_files), desc="Converting pcaps to json") as pbar, \
        ...
        for pcap_file in pcap_files:
            ...
            # runs convert_pcap_to_json(), whcih uses tshark to convert pcap to json
...
def run_pcap_splitter(pcap_file):
    # command uses PcapSplitter from pcapplusplus
...
def process_pcaps():
    ...
    for pcap_file in pcap_files:
        #Splits pcaps bassed on conversation
        run_pcap_splitter(pcap_file)
        # move the orignal pcaps, might want to delete to save space
...
if __name__ == "__main__":
    ...
    # gets the pcaps ready to convert to json
    process_pcaps()

    # converts pcaps to json, without any filtering 
    pcaps2jsons(jsons_output_directory)

    target_directory = "./jsons1"

    # moves jsons into respected directory 
    run_move_jsons(target_directory)
    ...
    # this code runs filtered_jsons() with multiple cores, commented out due to needing to fix filtered_jsons() first
    """with concurrent.futures.ThreadPoolExecutor(max_workers=num_cores) as executor:
        executor.map(process_directory, range(len(directory_protocol)), directory_protocol, output_directory_filtered)"""
    ...
    # untested at the moment, should replce the previous for loop
    """with Pool(num_processes) as pool:
        pool.map(combine_and_finalize_wrapper, args_list)"""
